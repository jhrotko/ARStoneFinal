...
public class FireDragonManager : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (!gm.PlayerTurn && timer >= timerMax){
            ...
            // Debug.Log("Collision with Zombie - gave attack, zombie life= " + col.gameObject.GetComponent<ZombieManager>().life);
            timer = 0.0f;
            gm.PlayerTurn = true;
        }
    }
    ...
    void OnCollisionEnter(Collision col)
    {
        if (col.gameObject.name == "Zombie" && gm.PlayerTurn)
        {
            ...
            Debug.Log("Collision with Zombie - received attack, dragon life= " + life);
            gm.PlayerTurn = false;
            Debug.Log("yo dragon");
            timer = 0.0f;
        }
        ...
        else if ( col.gameObject.name == "Fireball" && gm.PlayerTurn)
        {
            ...
            Debug.Log("Collision with Fireball - received attack, dragon life= " + life);
